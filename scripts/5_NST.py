...
# # train_dir_nst = base_dir + '\\class_train_NST\\'

# #%%
# train_dir_5 = base_dir + '\\class_train_5\\'
# train_dir_10 = base_dir + '\\class_train_10\\'
# train_dir_20 = base_dir + '\\class_train_20\\'
# train_dir_40 = base_dir + '\\class_train_40\\'
# train_dir_80 = base_dir + '\\class_train_80\\'
# train_dir_160 = base_dir + '\\class_train_160\\'
# train_dir_320 = base_dir + '\\class_train_320\\'
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self,  batch_size=batch_size): 
        ...
        self.train_dir_320 = train_dir_320
        self.train_dir_160 = train_dir_160
        self.train_dir_80 = train_dir_80
        self.train_dir_40 = train_dir_40
        self.train_dir_20 = train_dir_20
        self.train_dir_10 = train_dir_10
        self.train_dir_5 = train_dir_5
    ...
    def setup(self, stage=None):
        ...
        if self.train_percentage == number_560:
           ...
           val_dataset = Subset(full_dataset, val_indices)
            ...
            reduced_dataset = Subset(full_dataset, full_indices)

            # Determine the class distribution in the full dataset
            class_distribution = {class_label: 0 for class_label in range(len(full_dataset.classes))}
            for idx in full_indices:
                class_label = full_dataset[idx][1]  # Assuming the label is the second element in the tuple
                class_distribution[class_label] += 1
            
            # Create a validation dataset and a training dataset with the same distribution of classes
            val_indices = []
            train_indices = []
            for class_label, count in class_distribution.items():
                class_indices = [idx for idx in full_indices if full_dataset[idx][1] == class_label]
                train_idx, val_idx = train_test_split(class_indices, test_size=0.2, stratify=[class_label]*len(class_indices))
                val_indices.extend(val_idx)
                train_indices.extend(train_idx)
            
            # Create Subset datasets for validation and training
            val_dataset = Subset(full_dataset, val_indices)
            print("Size of val dataset", len(val_dataset))
            
          
            if self.train_percentage == number_320:                
                train_dataset = datasets.ImageFolder(root=self.train_dir_320, transform=self.transform)  
                print("Training dataset Size:", len(train_dataset))
            elif self.train_percentage == number_160:                
                train_dataset = datasets.ImageFolder(root=self.train_dir_160, transform=self.transform)  
                print("Training dataset Size:", len(train_dataset))
            elif self.train_percentage == number_80:                
                train_dataset = datasets.ImageFolder(root=self.train_dir_80, transform=self.transform)  
                print("Training dataset Size:", len(train_dataset))
            elif self.train_percentage == number_40:                
                train_dataset = datasets.ImageFolder(root=self.train_dir_40, transform=self.transform)  
                print("Training dataset Size:", len(train_dataset))
            elif self.train_percentage == number_20:                
                train_dataset = datasets.ImageFolder(root=self.train_dir_20, transform=self.transform)  
                print("Training dataset Size:", len(train_dataset))
            elif self.train_percentage == number_10:                
                train_dataset = datasets.ImageFolder(root=self.train_dir_10, transform=self.transform)  
                print("Training dataset Size:", len(train_dataset))
            elif self.train_percentage == number_5:                
                train_dataset = datasets.ImageFolder(root=self.train_dir_5, transform=self.transform)  
                print("Training dataset Size:", len(train_dataset))
            ...
            print("Class distribution final train dataset (original + NST augmented):")
...
class ResNet18(pl.LightningModule):
    def __init__(self, num_classes=num_classes):
        ...
        self.resnet152 = models.resnet152(weights=None)              
...
if __name__ == '__main__':
     ...
     for seed in seed_list:
        ...
        # [number_560, number_320, number_160, number_80, number_40, number_20, number_10, number_5]
        ...
        for train_percentage in train_p:     
            ...
            # Save the model
            model_save_path = os.path.join(save_dir, f"NST_CB_{seed}_model_{int(train_percentage*100)}.pth")
...
#%% CSV SAVING
        ...
        combined_df.to_csv(f"NST_LC_{seed}.csv", index=False)
        os.remove(f"NST_metrics.csv")
        
        combined_df.to_json(f"NST_LC_{seed}", orient="records")
