...
class DataModule(pl.LightningDataModule):
    def __init__(self, batch_size=batch_size, seed=42):
        super().__init__()
...
#%% CSV SAVING
        ...
        combined_df.to_csv(f"D_LC_{num_epochs}.csv", index=False)
        os.remove(f"D_metrics.csv")
        
        combined_df.to_json(f"D_LC_{num_epochs}", orient="records")
