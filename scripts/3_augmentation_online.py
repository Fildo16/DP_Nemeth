...
val_transform = transforms.Compose([
    transforms.Resize((224, 224)),  # Resize the image to the required input dimensions
...
class DataModule(pl.LightningDataModule):
    ...
    def __init__(self, batch_size=batch_size, train_transform = train_transform, val_transform = val_transform, test_transform = test_transform): 
        super().__init__()
    ...
    def setup(self, stage=None):
        ...
        full_dataset = datasets.ImageFolder(root=self.train_dir)
        ...
        print("Train - Number of samples per class", len(self.train_dataset.dataset)//2)
        
        self.val_dataset = DataLoader(val_dataset, batch_size=self.batch_size, shuffle=True)
        print("Validation - Number of samples per class", len(self.val_dataset.dataset)//2)
        
        self.test_dataset = DataLoader(test_dataset, batch_size=self.batch_size)
        # print("Number of samples per class", len(self.test_dataset.dataset))
...
class ResNet(pl.LightningModule):
    def __init__(self, num_classes=num_classes):
        ...
        self.resnet20 = models.resnet18(weights=None)              
        self.resnet20.fc = nn.Linear(self.resnet20.fc.in_features, num_classes) 

    def forward(self, x):
        return self.resnet20(x)
...
if __name__ == '__main__':
    ...
    save_dir = "./models/DA_online_models/CB"
    ...
    for seed in seed_list:
        ...
        for train_percentage in train_p:          
            ...
            # Save the model
            model_save_path = os.path.join(save_dir, f"DA_online_CB_{seed}_model_{int(train_percentage*100)}.pth")
...
#%% CSV SAVING
        ...
        combined_df.to_csv(f"DA_online_CB_{seed}.csv", index=False)
        os.remove(f"DA_metrics.csv")
        
        combined_df.to_json(f"DA_online_CB_{seed}", orient="records")
